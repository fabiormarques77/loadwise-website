...
export default function Contact() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      body.set("formSource", "loadwise_website_contact");
      submissionKey.current ||= `contact:${crypto.randomUUID()}`;
      const response = await fetch("/api/applications", {
        method: "POST",
        body,
        headers: { "x-idempotency-key": submissionKey.current },
      });
      if (!response.ok) {
  ...
  return <Shell>
    <section className="contact-hero wrap"><div><span className="hero-tag"><i/> {t("TALK TO OUR TEAM")}</span><h1>{t("Let’s talk about")}<br/>{t("your operation.")}</h1><p>{t("Tell LoadWise a little about your operation. It takes less than a minute, and our team will call you to discuss the next step.")}</p></div><div className="contact-paths"><a href="#request-call"><span>{t("FOR DRIVERS")}</span><h3>{t("Request a call")}</h3><p>{t("Owner-operators and small fleets are welcome.")}</p><b>{t("Tell us about your operation →")}</b></a><a href="#partners"><span>{t("FOR PARTNERS")}</span><h3>{t("Explore a partnership")}</h3><p>{t("Technology, freight, capital or ecosystem collaboration.")}</p><b>{t("Contact LoadWise →")}</b></a></div></section>
